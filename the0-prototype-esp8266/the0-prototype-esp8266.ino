...
// todo: //       -make the ESPs use short range bluetooth as backup sync method
...
#define DATA_PIN 2 // <<< the pin that sends rgb data to the led strip
// #define IR_RX_PIN 4 // <<< IR = infrared, RX = receiver
// #define IR_TX_PIN 5 // <<< TX = transmitter
...
void BUTTONFUNC2(){
  ...
  if (serialcommand == "palette") {
    Serial.println("palette ok");
  }
  if (serialcommand == "rainbow") {
    Serial.println("rainbow ok");
  }
}
...
void setup() { 
    ...
    Serial.begin(115200, SERIAL_8N1); //start serial connection
